...
router.post('/:taskId/subtasks', verifyToken, verifyAdmin, async (req, res) => {
  const { taskId } = req.params
  const { title, description = '', expected_date } = req.body
...
router.get('/:taskId/notes', verifyToken, async (req, res) => {
  const { taskId } = req.params
...
router.put('/subtask/:subtaskId/status', verifyToken, async (req, res) => {
  ...
  try {
    ...
    const result = await pool.query(
      `UPDATE subtasks SET status = $1 WHERE id = $2 RETURNING *`,
      [status, subtaskId]
    )
    if (!result.rows[0]) {
      logger.warn('tasks.updateSubtaskStatus', 'Subtask not found', { subtaskId })
    ...
    const allDone = allSubs.rows.length > 0 && allSubs.rows.every(s => s.status === 'completed')
    ...
    if (allDone) {
      ...
      await pool.query(
        `UPDATE tasks SET status = 'in_progress' WHERE id = $1 AND status = 'completed'`,
        [taskId]
      )
    }
